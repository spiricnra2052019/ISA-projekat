...
public class BloodCenterAdministratorController {
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "successful operation", content = @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = BloodCenterAdministrator.class))))
	})
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "201", description = "Created", content = {
					@Content(mediaType = "application/json", schema = @Schema(implementation = BloodCenterAdministrator.class)) }),
			@ApiResponse(responseCode = "409", description = "Not possible to create new Administrator when given id is not null", content = @Content)
	})
	@PostMapping(consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<BloodCenterAdministrator> createBloodCenterAdministrator(
			@RequestBody BloodCenterAdministrator administrator) {
	...
	@Operation(summary = "Get by id", description = "Get by id", method = "GET")
	@ApiResponses(value = {
	...
	public ResponseEntity<BloodCenterAdministrator> getAdministrator(@PathVariable("id") Long id) {
		BloodCenterAdministrator administrator = bloodCenterAdministratorService.findOne(id);
		if (administrator == null) {
			return new ResponseEntity<BloodCenterAdministrator>(HttpStatus.NOT_FOUND);
		}
		return new ResponseEntity<BloodCenterAdministrator>(administrator, HttpStatus.OK);
	}
